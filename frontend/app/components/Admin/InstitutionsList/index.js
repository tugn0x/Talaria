...
const InstitutionsList = (props) => {
    ...
    const { editPath,loading, data, pagination, searchOptions,deleteInstitution, changeStatusInstitution} = props
    ...
    const toggleAllCheckbox = (e) => {
        ...
        setSelectedInstitutions( chk ? [...data.map(inst => inst.id )] : [])
    }
            ...
            <div className="institutionsList list-wrapper">
                {<Row className="list-head">
                    ...
                    <div className="select-counter">
                        <FormattedMessage {...messages.InstitutionSelected} /> {selectedInstitutions.length} di {data.length} 
                    </div>
                </Row>}
                <Loader show={loading}>
                        ...
                        {data.length > 0 &&
                            data.map(inst => (
                                <InstitutionItem 
                                    ...
                                    data={inst}                                    
                                    editPath={editPath}
                                    toggleSelection={() => toggleInstitution(inst.id)}                                                                        
                                    checked={selectedInstitutions.includes(inst.id)}
                                    deleteInstitution={()=>deleteInstitution(inst.id,multiFilter)}
                                    changeStatusInstitution={(status)=>changeStatusInstitution(inst.id,status,multiFilter)}
                                    
                                />                                                                           
                            ))
                        ||                            
                            <h5 className="text-center">
                                {intl.formatMessage(messages.InstitutionsNotFound)}
                            </h5>
                        }
                    </div>
                </Loader>
